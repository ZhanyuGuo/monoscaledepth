...
class ResnetEncoderMatching(nn.Module):
    ...
    def __init__(
        ...
        self.reduce_conv = nn.Sequential(
            nn.Conv2d(
                self.num_ch_enc[1] + self.num_depth_bins,
    ...
    def match_features(self, current_feats, lookup_feats, relative_poses, K, invK):
        ...
        for batch_idx in range(len(current_feats)):
            ...
            cost_volume = torch.zeros(
                volume_shape, dtype=torch.float, device=current_feats.device
            )
            counts = torch.zeros(
                volume_shape, dtype=torch.float, device=current_feats.device
            )
            ...
            for lookup_idx in range(_lookup_feats.shape[1]):
                ...
                # convert from (-1, 1) to pixel values
                # fmt: off
                ...
                edge_mask = (
                    (x_vals >= 2.0)
                    * (x_vals <= self.matching_width - 2)
                    * (y_vals >= 2.0)
                    * (y_vals <= self.matching_height - 2)
                )
                ...
                diffs = (
                    torch.abs(warped - current_feats[batch_idx : batch_idx + 1]).mean(1)
                    * edge_mask
                )
            ...
            if self.set_missing_to_max:
                cost_volume = (
                    cost_volume * (1 - missing_val_mask)
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            lookup_images = lookup_images.reshape(
                batch_size * num_frames, chns, height, width
            )
            lookup_feats = self.feature_extraction(
                lookup_images, return_all_feats=False
            )
            _, chns, height, width = lookup_feats.shape
            lookup_feats = lookup_feats.reshape(
                batch_size, num_frames, chns, height, width
            )

            # warp features to find cost volume
            cost_volume, missing_mask = self.match_features(
                current_feats, lookup_feats, poses, K, invK
            )
        ...
        post_matching_feats = self.reduce_conv(
            torch.cat([self.features[-1], cost_volume], 1)
        )
