...
class kitti_raw_pose_loader(object):
    def __init__(self, dataset_dir, split, img_height=192, img_width=640, seq_length=3):
    ...
    def is_valid_sample(self, frames, tgt_idx):
        ...
        if (
            tgt_drive == min_src_drive
            and tgt_drive == max_src_drive
            and cid == min_src_cid
            and cid == max_src_cid
        ):
            return True
    ...
    def load_image_raw(self, drive, cid, frame_id):
        ...
        img_file = os.path.join(
            self.dataset_dir, date, drive, "image_" + cid, "data", frame_id + ".jpg"
        )
    ...
    def load_pose(self, drive, tgt_frame_id):
        ...
        for o in range(-half_offset, half_offset + 1):
            ...
            curr_pose_file = os.path.join(
                self.dataset_dir, date, "%s/poses/%.10d.txt" % (drive, curr_idx)
            )
